...
* **Form di iscrizione criptato**
  ...
  * Dati personali cifrati prima del salvataggio
  * Upload documento identità fronte/retro (PDF)
* **Invio automatico email al locale**

  * Il form, una volta completato, genera:

    * un **PDF con i documenti** (fronte/retro del documento del socio)
    * un **PDF per la tessera ACSI già compilato**, dove:

      * i dati del socio vengono posizionati sul template tramite **coordinate x/y**
      * il layout è pronto per la stampa/archiviazione
  * Entrambi i PDF vengono inviati via **email al locale** come allegati
* Storage privato Supabase per i file caricati e generati
...
* **Pulsante `Download file .xlsx`**:

  * Genera un file **.xlsx** con il tracciato dati conforme ai requisiti ACSI
  * Utilizzabile per **import massivo** dei soci sui sistemi ACSI
  * Pensato per ridurre errori manuali e velocizzare le pratiche di tesseramento
...
🔗 [https://www.linkedin.com/in/sofia-vidotto-junior-developer/](https://www.linkedin.com/in/sofia-vidotto-junior-developer/)
