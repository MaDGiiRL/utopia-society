 ...
 * Le colonne in chiaro (email, phone, fiscal_code) le lasciamo a NULL.
 */
export async function createMemberFromForm(rawBody) {
    ...
    const insertPayload = {
        ...
        // ❌ NIENTE dati sensibili in chiaro per i nuovi record
        email: null,
        phone: null,
        fiscal_code: null,
    ...
    // ritorno oggetto "pulito" (sempre decriptato) per il frontend
    return mapMemberRowDecrypted(data);
}

/**
 * Helper: mappa una row del DB in un oggetto pulito con campi decriptati.
 * Usa *_enc se presenti, altrimenti cade sui campi in chiaro (per vecchi record).
 */
